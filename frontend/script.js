...
function validateForm(data) {
  ...
  if (!data.creatinine || !data.bun || !data.gfr || !data.urineOutput)
    return false;
...
async function simulatePrediction(data) {
  ...
  if (data.gfr < 30) riskScore += 0.6;
  else if (data.gfr < 60) riskScore += 0.3;
  else riskScore += 0.05;

  // BUN (23.52%)
  if (data.bun > 40) riskScore += 0.2;
  else if (data.bun > 20) riskScore += 0.1;
  else riskScore += 0.02;
...
function updateRiskAssessment(riskScore) {
  ...
  if (riskScore < 0.3) {
    level = "Low Risk";
    description =
      "Patient shows minimal indicators of kidney disease. Continue regular monitoring.";
  } else if (riskScore < 0.6) {
    level = "Moderate Risk";
    description =
      "Patient shows some indicators of kidney disease. Increased monitoring recommended.";
  } else {
    level = "High Risk";
    description =
      "Patient shows significant indicators of kidney disease. Immediate medical attention recommended.";
  }
...
function updateFeatureImportance(features) {
  ...
  featureBars.innerHTML = featureImportance
    .map(
      (feature) => `
        <div class="feature-item">
            <span class="feature-name">${feature.name}</span>
            <div class="feature-bar">
                <div class="feature-bar-fill" style="width: ${
                  feature.weight
                }%"></div>
            </div>
  ...
  summaryGrid.innerHTML = `
        ...
        <div class="summary-item">
            ...
            <div class="summary-value">${
              patientData.diabetes ? "Yes" : "No"
            }</div>
        </div>
        <div class="summary-item">
            <div class="summary-label">Hypertension</div>
            <div class="summary-value">${
              patientData.hypertension ? "Yes" : "No"
            }</div>
        </div>
    `;
}
...
function updateRecommendations(prediction, patientData) {
  ...
  if (prediction.ckdStatus === 1) {
    ...
    if (patientData.gfr < 30) {
      recommendations.push(
        "GFR is critically low. Patient may require dialysis preparation."
      );
    } else if (patientData.gfr < 60) {
      recommendations.push(
        "GFR indicates Stage 3 CKD or higher. Close monitoring required."
      );
    }

    if (patientData.bun > 40) {
      recommendations.push(
        "Elevated BUN levels suggest reduced kidney function. Dietary modifications recommended."
      );
    }

    if (patientData.creatinine > 2.0) {
      recommendations.push(
        "High creatinine levels detected. Further diagnostic tests recommended."
      );
    }

    if (patientData.diabetes) {
      recommendations.push(
        "Strict blood sugar control is essential to prevent further kidney damage."
      );
    }

    if (patientData.hypertension) {
      recommendations.push(
        "Blood pressure management is critical for kidney health."
      );
    }
  } else {
    recommendations.push(
      "No immediate kidney disease detected. Continue regular health monitoring."
    );
    recommendations.push(
    ...
    if (patientData.diabetes || patientData.hypertension) {
      recommendations.push(
        "Continue managing existing conditions to prevent future kidney complications."
      );
    }
  }
...
function downloadReport() {
  alert(
    "Download report functionality will be implemented with the backend API."
  );
...
function filterPatients(filter) {
  ...
  rows.forEach((row) => {
    ...
    if (filter === "all") {
      ...
      filter === "no-ckd" &&
      status.classList.contains("badge-success")
    ) {
      row.style.display = "";
...
window.addEventListener("load", function () {
  const cards = document.querySelectorAll(
    ".form-card, .result-card, .dashboard-card"
  );
